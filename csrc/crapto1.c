...
static uint8_t crapto1_bit(struct Crapto1State *s, uint8_t in, int is_encrypted) {
  uint32_t feedin, t;
...
static inline uint32_t crypt_word_par(struct Crapto1State *s, uint32_t in, int is_encrypted,
                                      uint32_t nt_plain, uint8_t *parity_keystream_bits) {
  uint32_t ret = 0;
  *parity_keystream_bits = 0;

  for (int i = 0; i < 32; i++) {
    uint8_t bit = crapto1_bit(s, BEBIT(in, i), is_encrypted);
    ret |= bit << (24 ^ i);
    if ((i + 1) % 8 == 0) {
...
static inline uint32_t crypt_word(struct Crapto1State *s) {
  uint32_t res_ret = 0;
  uint32_t feedin, t;
  for (int i = 0; i <= 31; i++) {
...
static inline void crypt_word_noret(struct Crapto1State *s, uint32_t in, int x) {
  uint8_t ret;
...
static inline uint32_t crypt_word_ret(struct Crapto1State *s, uint32_t in, int x) {
  uint32_t ret = 0;
  uint32_t feedin, t, next_in;
  uint8_t next_ret;
  for (int i = 0; i <= 31; i++) {
...
static inline void rollback_word_noret(struct Crapto1State *s, uint32_t in, int x) {
  uint8_t ret;
  uint32_t feedin, t, next_in;
  for (int i = 31; i >= 0; i--) {
    next_in = BEBIT(in, i);
...
static uint8_t napi_lfsr_rollback_bit(struct Crapto1State *s, uint32_t in, int fb) {
  int out;
  uint8_t ret;
  uint32_t t;
  s->odd &= 0xffffff;
...
static uint32_t napi_lfsr_rollback_word(struct Crapto1State *s, uint32_t in, int fb) {
  int i;
  uint32_t ret = 0;
  for (i = 31; i >= 0; --i)
...
static void crapto1_get_lfsr(struct Crapto1State *state, uint8_t *lfsr_out6) {
  int i;
...
static inline int check_state(struct Crapto1State *t, RecoverCtx *ctx) {
  if (!(t->odd | t->even))
    return 0;
